    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="mx-auto max-w-4xl">
        <div className="flex justify-between mb-8">
          <h1 className="text-3xl font-bold text-gray-800">{dictionary.roles.tenant} {dictionary.navigation.dashboard}</h1>
          <Link href={`/${lang}`} className="flex-shrink-0">
            <Button variant="outline" className="text-red-600">{dictionary.auth.signOut}</Button>
          </Link>
        </div>
          ...
          <Card className="mb-8">
            <div className="p-4">
              <div className="flex justify-between items-start">
                ...
                >
                  {isExpanded ? dictionary.common.back : dictionary.common.moreDetails}
                </Button>
              </div>
              {isExpanded && (
                <>
                  ...
                  <div className="mt-4 pt-4 border-t">
                    <div className="flex justify-between items-center">
                      <Link href={`/${lang}/tenant/announcements`}>
                        <Button variant="outline" size="sm">
                          {dictionary.navigation.announcements}
                        </Button>
                      </Link>
                      <Link href={`/${lang}/tenant/contact`}>
                        <Button variant="outline" size="sm">
                          {dictionary.tenant.contactLandlord}
                        </Button>
                      </Link>
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          >
            ...
            <h2 className="text-lg font-semibold text-gray-800">{dictionary.tenant.contactLandlord}</h2>
          </Link>

          <Link
            href={`/${lang}/tenant/rent`}
            className="flex flex-col items-center justify-center rounded-lg bg-white p-6 shadow-md transition-all hover:shadow-lg"
          >
            ...
            <h2 className="text-lg font-semibold text-gray-800">{dictionary.tenant.currentRent}</h2>
          </Link>

          <Link
            href={`/${lang}/tenant/lease`}
            className="flex flex-col items-center justify-center rounded-lg bg-white p-6 shadow-md transition-all hover:shadow-lg"
          >
            ...
            <h2 className="text-lg font-semibold text-gray-800">{dictionary.tenant.leaseAgreement}</h2>
          </Link>

          <Link
            href={`/${lang}/tenant/maintenance`}
            className="flex flex-col items-center justify-center rounded-lg bg-white p-6 shadow-md transition-all hover:shadow-lg"
          >
            ...
            <h2 className="text-lg font-semibold text-gray-800">{dictionary.tenant.maintenance}</h2>
          </Link>
        </div>
      </div>
    </div>
